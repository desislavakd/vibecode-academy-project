...
│   └── frontend/
│       ├── next.config.ts
│       ├── proxy.ts                   ← Edge Runtime — protects /dashboard/admin/*
│       ├── app/
│       │   ├── layout.tsx             ← Space Grotesk + DM Sans fonts
│       │   ├── globals.css            ← entry point — @import only (17 lines)
│       │   ├── base.css               ← reset, design tokens (:root), typography
│       │   ├── layout/
│       │   │   ├── header.css         ← header, logo, .page, .card
│       │   │   └── sidebar.css        ← topbar, sidebar, dashboard shell
│       │   ├── components/
│       │   │   ├── card.css           ← tool card, glass card, animated border
│       │   │   ├── form.css           ← inputs, tool form, tag autocomplete
│       │   │   ├── buttons.css        ← .btn variants (primary, outline, danger…)
│       │   │   ├── alerts.css         ← alert-error, alert-success
│       │   │   └── ratings.css        ← rating widget + card badge
│       │   ├── pages/
│       │   │   ├── auth.css           ← login page
│       │   │   ├── tools.css          ← catalogue filter bar, chips, pagination
│       │   │   ├── tool-detail.css    ← hero card, breadcrumb, sections
│       │   │   ├── dashboard.css      ← profile card, quick actions, carousel
│       │   │   ├── admin.css          ← admin badges + full audit log
│       │   │   └── settings.css       ← profile rows, 2FA
│       │   ├── login/page.tsx
│       │   └── dashboard/
...
│       │           ├── new/page.tsx   ← thin wrapper around ToolForm
│       │           ├── [id]/page.tsx  ← tool detail — interactive star rating widget
│       │           └── edit/[id]/page.tsx  ← thin wrapper around ToolForm
│       │       ├── admin/page.tsx     ← Admin Panel (approve / reject)
│       │       ├── admin/audit/page.tsx  ← Audit Log feed
│       │       └── settings/page.tsx  ← Two-Factor Auth
│       ├── components/
│       │   ├── Sidebar.tsx            ← navigation; admin link visible to owner only + pending-count badge
│       │   ├── ToolForm.tsx           ← shared create/edit form (mode: 'create' | 'edit')
│       │   └── Pagination.tsx         ← reusable pagination buttons
│       └── lib/
│           ├── auth.ts                ← login, logout, getUser, 2FA helpers
│           ├── tools.ts               ← getTools, approveTool, rejectTool, getAuditLogs, …
│           ├── constants.ts           ← ALL_ROLES, roleColors, catPalette (single source of truth)
│           └── utils.ts               ← getFaviconUrl, handleTilt, handleTiltReset
│
